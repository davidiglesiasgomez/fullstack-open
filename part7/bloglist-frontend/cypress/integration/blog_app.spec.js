...
describe('Blog app', function() {
  ...
  describe('Order',function() {
    beforeEach(function() {
      cy.login({ username: 'root', password: 'root' })
      cy.createBlog({ title: 'First blog title', author: 'First blog author', url: 'foo.bar.com' })
    ...
    it('Blogs order by number of likes', function() {
      ...
      cy.get('strong').eq(0).contains('First blog title')
      cy.contains('Second blog title').parent().find('button.likeBlogButton').click()
      cy.reload()
      cy.get('button.toggleBlogButton').each((el) => {
        cy.wrap(el).click()
      })
      cy.get('strong').eq(0).contains('Second blog title')
    })
  })
})